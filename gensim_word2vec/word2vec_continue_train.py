...
logging.info('before new model vocab size ', len(model_new.wv.vocab.keys()))
total_examples = model_new.corpus_count
logging.info('total corpus size ', total_examples)
...
# logging.info('before pre model vocab size ', len(list(model_pre.vocab.keys())))
#
# # 重新训练
# model_new.build_vocab([list(model_pre.vocab.keys())], update=True)
# logging.info('after new model vocab size ', len(model_new.wv.vocab.keys()))
#
